# -*- coding: utf-8 -*-
# @Author: watcher
# @Created Time: 2023/3/23 4:25 PM
# @File: bert_classification_model
...
import pandas as pd
import torch
from EasyAI.common import logger
...
sys.path.append("../../..")
from EasyAI.TextClassifier.transformer.utils.bert_classification_utils import (
...
from EasyAI.TextClassifier.transformer.models.bert_multi_label_classification_model import (
...
class BertClassificationModel:
        ...
        if not multi_label:
            if self.args.labels_list:
                ...
                if self.args.labels_map:
                    try:
                        assert (
                            sorted(list(self.args.labels_map.keys()))
                        ...
                        self.args.labels_map = {
                            int(key): value
                            for key, value in self.args.labels_map.items()
                        }
                else:
                    self.args.labels_map = {
    ...
    def predict(self, to_predict, top_k: int = None):
        """
        Performs predictions on a list of text.

        Args:
            to_predict: A python list of text (str) to be sent to the model for prediction.
            top_k: int, if not None return top_k
        ...
        if self.multi_label:
            ...
            if top_k is None:
                ...
                out_label_ids = np.empty((len(to_predict), top_k))
    ...
    def _calculate_loss(self, model, inputs, loss_fct, num_labels, args):
        ...
        if loss_fct:
            ...
            if self.multi_label:
                loss = loss_fct(
                    logits.view(-1, num_labels), labels.view(-1, num_labels)
                )
            else:
