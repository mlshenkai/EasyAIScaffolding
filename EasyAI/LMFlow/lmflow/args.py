# -*- coding: utf-8 -*-
# @Author: watcher
# @Created Time: 2023/4/11 7:49 PM
# @File: args
...
class ModelArguments:
    ...
    trust_remote_code: bool
        a boolean indicating whether to trust_remote_code
    ...
    arch_type: Optional[str] = field(
        ...
        metadata={
            "help": (
                'Model architecture type, e.g. "decoder_only",' ' "encoder_decoder"'
            ),
    ...
    lora_r: int = field(
        default=8,
        metadata={
            "help": "the rank of the lora parameters. The smaller lora_r is , the fewer parameters lora has."
        },
    )
    ...
    trust_remote_code: bool = field(
        default=False, metadata={"help": "whether to trust_remote_code"}
    )
...
class FinetunerArguments(TrainingArguments):
    """
    References transformers.TrainingArguments
    """
...
class AutoArguments:
    """
    Auto chose through argument name
    """

    @classmethod
    def get_pipeline_args_class(cls, pipeline_name: str):
        if pipeline_name not in PIPELINE_ARGUMENT_MAPPING:
            raise KeyError(
                f"{pipeline_name} not in {PIPELINE_ARGUMENT_MAPPING}, please insure pipeline name"
            )
