...
class LMDecodeModel(LMBaseModel):
    ...
    def __init__(
        ...
        super(LMDecodeModel, self).__init__(model_args, tune_strategy, ds_config, use_gpu, *args, **kwargs)
        ...
        if tune_strategy == "none":
            ...
            if model_args.use_ram_optimized_load and peft_model_id is None:
                try:
                    ...
                    self.backend_model = AutoModel.from_pretrained(
                        model_args.model_name_or_path, trust_remote_code=model_kwargs.get("trust_remote_code", False)
                    )
            else:
